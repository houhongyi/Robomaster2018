...
USART_RECEIVETYPE UsartType1={0},UsartType2={0},UsartType3={0},UsartType4={0},UsartType5={0},UsartType6={0}; 
...
xQueueHandle UART5_TXData_QueHandle;//UART发送队列
xQueueHandle UART5_RXData_QueHandle;//UART接收队列
...
//UART5DMA发送
void Usart5SendData_DMA(uint8_t *pdata, uint16_t Length)  
{  
    while(UsartType5.dmaSend_flag == USART_DMA_SENDING);  
    UsartType5.dmaSend_flag = USART_DMA_SENDING;  
    HAL_UART_Transmit_DMA(&huart5, pdata, Length);  
}
//UART6DMA发送
...
{  
     __HAL_DMA_DISABLE(huart->hdmatx);
		...
		if(huart->Instance==huart5.Instance) 
			UsartType5.dmaSend_flag = USART_DMA_SENDOVER;
...
//UART发送任务
void vUART5Sent_Task(void *pvParameters)
{
	char s[RECEIVELEN]={0};
	int n=0;
	float time=0;
	...
	while(1)
	{
		xQueueReceive(UART5_TXData_QueHandle,s,portMAX_DELAY);
		n=0;
		while(UsartType5.dmaSend_flag == USART_DMA_SENDING){
			n++;
			if(n>=20)
		...
		Usart5SendData_DMA((uint8_t *)s, strlen(s));
		UsartType5.Send_n++;
		time=GetSystemTime();
		if(time-UART_SendPRETime)
			UsartType5.Send_fps=1000.0f/(time-UART_SendPRETime);
		UART_SendPRETime=time;
	}
}
//UART分析任务
void vUART5Analy_Task(void *pvParameters)
{
	char s[RECEIVELEN]={0};
	while(1)
	{
		xQueueReceive(UART5_RXData_QueHandle,s,portMAX_DELAY);
		//UART信息处理
		UARTAnalys(&huart5,s,strlen(s));
	}
}
//=========UART6
//UART6发送任务
...
int UART_Inite()
{
	HAL_UART_Receive_DMA(&huart1, UsartType1.usartDMA_rxBuf, RECEIVELEN); //启动DMA接收
  ...
  __HAL_UART_ENABLE_IT(&huart4, UART_IT_IDLE);//使能串口中断
	...
	HAL_UART_Receive_DMA(&huart5, UsartType5.usartDMA_rxBuf, RECEIVELEN); //启动DMA接收
  __HAL_UART_ENABLE_IT(&huart5, UART_IT_IDLE);//使能串口中断
	
	HAL_UART_Receive_DMA(&huart6, UsartType6.usartDMA_rxBuf, RECEIVELEN); //启动DMA接收
  __HAL_UART_ENABLE_IT(&huart6, UART_IT_IDLE);//使能串口中断
	
	UART1_TXData_QueHandle=xQueueCreate(5,RECEIVELEN);//信息队列初始化 深度10 长度RECEIVELEN字节
	UART1_RXData_QueHandle=xQueueCreate(5,RECEIVELEN);
	...
	UART5_TXData_QueHandle=xQueueCreate(5,RECEIVELEN);//信息队列初始化 深度10 长度RECEIVELEN字节
	UART5_RXData_QueHandle=xQueueCreate(5,RECEIVELEN);
	...
	if(NULL==UART5_TXData_QueHandle && NULL==UART5_RXData_QueHandle)return 0;
	...
	//启动UART分析任务
	if(!xTaskCreate( vUARTAnaly_Task, "UART1Analy_Task", 200, NULL, UART_AnalyTask_Priority, NULL ))return 0;
	...
	//启动UART发送任务
	if(!xTaskCreate( vUART5Sent_Task,  "UART5Sent_Task" , 200, NULL, UART_SentTask_Priority , NULL ))return 0;
	//启动UART分析任务
	if(!xTaskCreate( vUART5Analy_Task, "UART5Analy_Task", 200, NULL, UART_AnalyTask_Priority, NULL ))return 0;
...
{  
   ...
   if(huart->Instance==huart1.Instance)
   {
     ...
     {   
        ...
        UsartType1.rx_len =  RECEIVELEN - temp; 
			  ...
			  memset(UsartType1.usartDMA_rxBuf,0,RECEIVELEN);
   ...
   if(huart->Instance==huart5.Instance)
   {
     ...
     {   
        ...
        UsartType5.rx_len =  RECEIVELEN - temp;   
				xQueueSendToBackFromISR(UART5_RXData_QueHandle,UsartType5.usartDMA_rxBuf,0);
			  memset(UsartType5.usartDMA_rxBuf,0,RECEIVELEN);
        HAL_UART_Receive_DMA(&huart5,UsartType5.usartDMA_rxBuf,RECEIVELEN);  
     }
   }
	 //==============UART6
   if(huart->Instance==huart6.Instance)
...
{
	return xQueueSendToBackFromISR(UART6_TXData_QueHandle,s,0);
}
int My_print(char* s)
{
	return xQueueSendToBack(UART6_TXData_QueHandle,s,0);
}

int UART_SendToQueue(UART_HandleTypeDef *huart,char* data)
{
	if(huart->Instance==huart1.Instance)
		return xQueueSendToBack(UART1_TXData_QueHandle,data,0);
	if(huart->Instance==huart2.Instance)
	...
	if(huart->Instance==huart5.Instance)
		return xQueueSendToBack(UART5_TXData_QueHandle,data,0);
	if(huart->Instance==huart6.Instance)
...
{
	if(huart->Instance==huart1.Instance)
		return xQueueSendToBackFromISR(UART1_TXData_QueHandle,data,0);
	if(huart->Instance==huart2.Instance)
	...
	if(huart->Instance==huart5.Instance)
		return xQueueSendToBackFromISR(UART5_TXData_QueHandle,data,0);
	if(huart->Instance==huart6.Instance)
...
//
//================================================
void GY_53UartSent(GY_53Def* temGY_53,char* s)
{
		if(temGY_53==&GY_53_1)
				UART_SendToQueue(&huart1,s);
		if(temGY_53==&GY_53_2)
				UART_SendToQueue(&huart2,s);
		if(temGY_53==&GY_53_3)
				UART_SendToQueue(&huart3,s);
		if(temGY_53==&GY_53_4)
				UART_SendToQueue(&huart4,s);
		if(temGY_53==&GY_53_5)
				UART_SendToQueue(&huart5,s);
		if(temGY_53==&GY_53_6)
				UART_SendToQueue(&huart6,s);
