...
{
	//state MotorPur_Lift
	motorxdef->State.No=MotorPur_Rotate;//点击用途 提升机构
	motorxdef->State.State = 0;
	motorxdef->State.Spd = 0;
	motorxdef->State.Loc = 0;
	motorxdef->State.Pre_MLoc = 0;
...
void Motor_Inite_All_YunTai(MotorXDef *motorxdef)
{
	...
	motorxdef->State.No=MotorPur_Yuntai;//点击用途 地盘驱动
	motorxdef->State.State = 0;
	motorxdef->State.Spd = 0;
	motorxdef->State.Loc = 0;
	motorxdef->State.Pre_MLoc = 0;
...
void SentControlData()
{
	//  遥控器 RC_Data.switch_right==2 就是失能
		...
		if(!(SystemState.Enable&SystemState_Enable_Lift)||RC_Data.switch_right==2)
			 memset(data,0,8);
		Can_Sent_msgToQue(2,0x200,data);//一级抬升电机
		...
		Can_Sent_msgToQue(2,CAN_YunTai_RECEIVE_ID,data);//夹子与 弹舱盖
		
		//扩展版1 夹子与抬升架
		memset(data,0,8);
		AddDeadZone(&data[0],SystemState.Robot.Catch,0);
		AddDeadZone(&data[2],SystemState.Robot.GetBulletFrame,0);
		AddDeadZone(&data[6],SystemState.Robot.EleBreak,0);//电磁制动器
		Can_Sent_msgToQue(2,CAN_EXTEND_RECEIVE_ID,data);//夹子与抬升架
		...
		if(!MyFlagGet(SystemState.OutLine_Flag,(MotorTotal_No+DisMesur_No)))//如果测距板上线 发送心跳包
		{
			memset(data,0,8);
			Can_Sent_msgToQue(1,CAN_DisMesure_RECEIVE_ID,data);//测距板心跳包
		}
}
//=================================================================
//
...
void vMotorPIDCtr_Task(void *pvParameters)
{
	...
	while(1)//电机控制
	{
		...
		Motor_control_UD(&MotorA,MotorA.pid_control_type);//抬升
		Motor_control_UD(&MotorB,MotorB.pid_control_type);//抬升
		Motor_control_UD(&MotorC,MotorC.pid_control_type);//旋转
		Motor_control_UD(&MotorD,MotorD.pid_control_type);//子弹导流板
		
		Motor_control_UD(&Motor5,Motor5.pid_control_type);//旋转
		Motor_control_UD(&Motor6,Motor6.pid_control_type);//子弹导流板
		//=======================发送控制指令=============
