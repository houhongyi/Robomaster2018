...
void vUART3Sent_Task(void *pvParameters)
{
	...
	while(1)
	{
		...
		if(s[0]=='<' && s[2]=='>')//如果帧头出现 <x> 则本次发送长度为x字节，从帧头后算起
...
void vUART4Sent_Task(void *pvParameters)
{
	...
	while(1)
	{
		...
		if(s[0]=='<'&&s[2]=='>')
			Usart4SendData_DMA((uint8_t *)s+3, s[1]);
		else
...
//================================================
//模拟示波器数据填充函数
void VisualScope_DataFill(char*s,short a,short b,short c,short d)
{
	s[0]=a&0xff;
	s[1]=a>>8;
...
{
	UART_SendToQueue(&huart6,(char*)s);
}
