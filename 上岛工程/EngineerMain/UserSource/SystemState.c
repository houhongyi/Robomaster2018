...
MotorXDef* Motors[MotorTotal_No]={&Motor1,&Motor2,&Motor3,&Motor4,&MotorA,&MotorB,&MotorC,&MotorD,&Motor5,&Motor6};//车上所有电机数组
...
void MotorStall_Check()
{
	...
	{
		if(my_abs(Motors[num]->State.Spd)<Motor_Stall_Spd && my_abs(Motors[num]->Motor_output)>Motors[num]->Protect.OutPut_Stall)//如果一个电机输出大于堵转保护值 而速度小于5mm/s 认为堵转
		{
			if(!g_Time_MotorStall[num])
				g_Time_MotorStall[num]=GetSystemTimer();//记录堵转时间
		}
		else
...
void OutLine_Check()
{
	...
	for(num=0;num<MotorTotal_No;num++)
	{
		...
		{
			ClearMotorOutLine(Motors[num]->State.State);//清除堵转标志
...
int SystemState_Inite()
{
	...
	SystemState.Robot.YunTai.Direct=Forward;//云台方向为前进
