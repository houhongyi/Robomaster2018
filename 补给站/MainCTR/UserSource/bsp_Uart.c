...
USART_RECEIVETYPE UsartType1={0},UsartType2={0},UsartType3={0},UsartType4={0},UsartType6={0}; 
...
void vUARTSent_Task(void *pvParameters)
{
	...
	while(1)
	{
		...
		if(s[0]=='<'&&s[2]=='>')
			UsartSendData_DMA((uint8_t *)(s+3), s[1]-'0');
		else
...
void vUART3Sent_Task(void *pvParameters)
{
	...
	while(1)
	{
		...
		if(s[0]=='<'&&s[2]=='>')
			Usart3SendData_DMA((uint8_t *)(s+3), s[1]-'0');
		else
...
void vUART4Sent_Task(void *pvParameters)
{
	...
	while(1)
	{
		...
		if(s[0]=='<'&&s[2]=='>')
			Usart4SendData_DMA((uint8_t *)(s+3), s[1]-'0');
		else
...
{
	return xQueueSendToBackFromISR(UART1_TXData_QueHandle,s,0);
}
int My_print(char* s)
{
	return xQueueSendToBack(UART1_TXData_QueHandle,s,0);
}
...
//
//================================================
void PN532_UART_Send(PN532ReaderDef *hPN532,unsigned char* d,short len)
{
	...
	if(hPN532==&PN532ReaderL)
	{
		s[0]='<';s[1]=len+'0';s[2]='>';
		memcpy(&s[3],d,len);
		UART_SendToQueue(&huart3,(char*)s);
	}
